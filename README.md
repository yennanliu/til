...
```
/claude-api prompt-audit 是 Anthropic 針對 Claude API 與 Claude Code 推出的提示詞審查與優化工具，
旨在幫助開發者清理過時或冗長的系統提示詞，進而降低 API 成本並提升模型準確率。
```
...
| 比較項目 | 10-K（Form 10-K） | 10-Q（Form 10-Q） |
| --- | --- | --- |
| **報告週期** | **年度**（每年提交 1 次） | **季度**（每年提交 3 次，第四季併入 10-K） |
| **審計狀態** | **經外部獨立會計師審計**（Audited） | **未經審計**（Unaudited，僅經內部審查） |
| **內容詳盡度** | 極高，包含完整的業務營運、市場風險、法律訴訟及管理層討論（MD&A） | 較簡略，主要更新該季度的財務數據變化與重大事件 |
| **提交期限** | 會計年度結束後 60 至 90 天內 | 財季結束後 40 至 45 天內 |

核心差異在於：**10-K** 提供企業全年的完整健檢報告，最具權威性與深度；**10-Q** 則是季度的營運追蹤，方便投資人即時掌握最新財務動態。
...
- **LiteLLM** 著重於**程式開發**，提供統一的 API 介面，讓開發者能輕鬆切換不同的 LLM 供應商。
- **LiteLLM Gateway** 則著重於**企業級管理**，提供集中化的 API 管理、安全控管、流量控制、成本監控與模型路由等功能，適合多團隊或大規模部署的環境。
````
...
```
/earnings-preview（財報前瞻）： 在公司公布財報前，進行情境分析與關鍵數據預測。讓你能提前做好推演，當數據開出來時，立刻判斷是「超預期」還是「低於預期」。
/earnings（財報後更新）： 財報公布後，秒級產出季度更新報告，抓出電話會議與財務數字的核心重點，不用再一頁頁啃幾十頁的原文 PDF。
2. 機構級的「深度研究與論點維護」
建立投資部位不是看線圖就好，背後的「基本面研究」才是持股底氣的來源：
/initiate（首次覆蓋報告）： 產出機構等級的首次覆蓋報告。對一檔新股票進行全面的基本面健檢與估值分析。
/model-update（更新財務模型）： 隨著新數據注入，自動更新 Excel 或量化模型中的財務參數。
/thesis（維護投資論點）： 投資最怕「抱著錯誤的期待」。這個功能可以幫你定期檢視持股核心邏輯是否有變，進行投資論點的動態修正。
3. 每日交易的「靈感與催化劑追蹤」
這組指令最適合用來做為每日盤前準備與選股策略：
/morning-note（晨會筆記）： 快速整理昨夜市場動態，產出今日的交易想法。
/sector（產業研究）與 /screen（選股篩選）： 從宏觀的產業概況到微觀的條件篩選，幫你源源不絕地提供新的投資點子。
/catalysts（催化事件追蹤）： 精準盯緊手工具體股票即將發生的重大事件（如法說會、新產品發表、臨床解盲等），提早卡位行情。
```
...
```
- 一個任務一個 session — 話題切換後，舊對話歷史變成每輪付費的噪音。新任務開新 session，舊 session 讓它留著。
- 批量提問，一次說完 — 「幫我整理這個檔案，列出重點，建議標題」一條訊息搞定。拆成三條 = 三次完整上下文重傳。
- 主動 /compact — 完成一個子任務就手動壓縮，附上你想保留的重點。不要等系統自動壓縮把重要東西丟掉。
- 給精確路徑，不要讓 AI 猜 — 「讀 src/services/auth.ts」比「幫我找處理登入的檔案」省很多。模糊描述會觸發多輪搜索，每輪都是一次 API 呼叫。
- 認真寫好 CLAUDE.md — 這份檔案每次對話都會被載入，第一輪付寫入費（貴 25%），之後每輪只付一折。5,000 token 的 CLAUDE.md 對話 20 輪，比每次手動提供同樣的資訊省將近 90%。你的 CLAUDE.md 還只有一行「請用繁體中文回答」嗎？
--
這次的學習讓我更確信一件事：AI 工具的成本管理，本質上是一個操作紀律的問題，不是技術問題。
你不需要看懂 Transformer 的注意力機制，也不需要讀 Claude Code 的原始碼。你只需要知道三個原則：
保持穩定 — 不要頻繁切模型、改設定、開關 session
前期投資 — 花時間寫好 CLAUDE.md，後面每一輪都在回收
控制增長 — 一任務一 session，主動壓縮，別讓上下文無限膨脹
同樣的 Max 套餐，操作習慣對了，可以多撐 3-5 倍。這不是省 token，是讓每一塊錢花在刀口上。
如果你的團隊正在導入 AI 工具，比起「要不要買更貴的方案」，更該先問一個問題 — 我們有 AI 使用的 SOP 嗎？
PS. 有人用 tmux 每 55 分鐘自動送一句「我斷線了嗎？」來幫快取續命。這大概是我見過最節儉的 keep-alive 方案了。
```
...
- block - `From Hierarchy to Intelligence.`
  ...
  - https://www.facebook.com/share/p/14bdRibHDfz/
```
以下是該文章的**繁體中文重點摘要**（精簡且有結構）：

---

## 一、核心論點

這篇文章（由 Jack Dorsey 等人撰寫）主張：

👉 **企業的本質問題是「資訊流動」問題**
👉 **傳統「階層式組織」本質上是用來傳遞資訊的機制**
👉 **AI 的出現，讓這種階層不再必要**

過去公司用「管理層」來傳遞與整合資訊，但未來可以由 AI 系統取代。 ([Project Flux][1])
...
* **羅馬軍隊**：建立多層級指揮（8 → 80 → 5000人）
* **普魯士參謀制度**：出現「中層管理」專責資訊處理
* **鐵路公司**：導入第一個企業組織圖
* **泰勒科學管理**：強化分工與效率
* **矩陣組織 / 現代企業**：試圖平衡複雜性
...
👉 所有組織創新，其實都在試圖解這個問題，但一直沒成功 ([CodeNote][2])
...
> 階層（Hierarchy） → 智能系統（Intelligence）

---

## 四、Block 提出的新組織模型（4 層架構）

### 1️⃣ Capabilities（能力模組）

* 基礎金融能力（支付、貸款等）
* 不直接面向使用者

### 2️⃣ World Model（世界模型）
...
👉 **AI 不是讓公司更有效率，而是讓公司「變成另一種東西」**

如果公司沒有建立「深度理解（world model）」：

* AI 只會變成成本削減工具

但如果有：

* AI 會成為企業的核心競爭力與運作方式 ([Block][3])

---

## 一句話總結

👉 **傳統公司是「人＋階層來傳遞資訊」，未來公司是「AI 系統直接理解與協調」，人只負責關鍵決策與現實接觸。**
```
...
```
Block（Square / Cash App / Afterpay 母公司）2月做了40%的裁員
，減少了4000人，上了新聞。
看到Block高層的專訪，太精彩了！趕快抓主要論點出來！一起打造未來，他們看到了什麼？建議按讚留友看，Youtube連結放留言中
--------------
why?
答：
2024 年底到 2025 年初，內部觀察到「少數上手 AI 工具的工程師」生產力可達 10–100 倍，傳統「人數 vs 輸出」的關聯在短時間內被打破。
---
how?
答：
Block 在 Q1 以「AI 時代一個理想組織應該長什麼樣」為出發點，自上而下重新設計組織，而不是單純用財務目標去倒推裁員比例。
----
who?
答：減幅超過 40%，重點集中在產品與開發端；合規、法遵等風險較高區塊幾乎不動，以確保系統穩定和監管關係。
---
so what?
答：新的組織與工作方式
從傳統 BU + 階層制，改成功能型、以 小 squad 為單位：1–6 人一組，可以在不同產品之間流動，取代過去「一個大團隊綁在一個產品線」的模式。
大幅減少管理層級與會議數量（減 70–80%），讓 PM / 設計 / 工程師都直接「自己 push PR」，回到「回頭寫程式、直接 shipping」的文化。
真實的日常變成「同時管理 10–20 個 AI agent 背景跑任務」，人類改成在多個 PR / output 之間來回審查、調整、合併，而不是線性流程。
----
how?
答：內部 AI 基礎設施：Goose、G2、Builder Bot
Goose：內部的 agent harness，模型不可知（支援 Anthropic、OpenAI、開源等約 120 種），大量自動化工作流都透過 Goose 路由。
G2：內部「agentic operating system」，讓任何員工可以把可確定的流程變成自動化 agent，串接到業務與營運流程裡。
Builder Bot：會自動寫 code、開 PR 並 merge；複雜功能有時可完全自動完成，更多時候做到 80–90%，最後 10% 由具備 domain context 的人類收尾。
客戶服務、風控、合規、營運等所有「判斷相對可制度化」的 queue work 正被大量自動化，短期仍強調「human in the loop」，但長期預期 agent 會比大規模人力更好更穩。
產品層：從靜態 UI 到生成式 UI
Block 已從 BU 結構改為跨品牌的平台：金融平台、業務平台等，都同時服務 Square、Cash App、Afterpay，讓 AI 能在整個生態系共用底層能力。
Moneybot（偏「口袋 CFO」）與 Manager Bot（偏商家營運助理）都建構在 Goose 之上，可以主動出擊、讀取數據、為用戶採取動作。
----
future
答：Block 把未來界面形態定義為 Generative UI：
每個人的 Cash App / 管理介面都會依實際需求動態生成，不只是一點個人化，而是「畫面與互動是即時生成」。
例如 Moneybot 會即時產生你的消費圖表與視覺化，但這些 chart 並不是事先寫死在前端程式碼中。
Manager Bot 可依餐廳老闆要求，自動生成排班＋簡訊通知系統的 app，UI/flow 不是硬編碼，而是在 runtime 被生成。
這帶來巨大的 QA 挑戰：如何在數千萬使用者都看到不一樣、且非決定性的 UI 時，保證安全與品質。
----
護城河在哪？
答：他們眼中的 moat 與長期觀點
短中期的護城河仍包括：分發與網路效應（數千萬活躍用戶難以「vibe code」出來）、監管與牌照、硬體佈局等。
真正長期的 moat 在於「對某些難以被他人理解的現象有獨特、持續加深的理解」，並能用 AI 工具高週期迭代這份理解。
Block 正在建構「公司本身的 world model」：
一方面是對客戶與經濟行為的深度模型。
另一方面是對 Block 自身（價值觀、指標、優先順序）的 markdown 式描述，再配合像 Builder Bot 這種 agent 不斷迭代實驗與 shipping。
當迭代週期從「幾個月做出一個 feature」變成「一天跑上百、上千次 loop」，公司就變成一個可快速自我改善的智能系統；若一家公司說不出自己獨特理解的是什麼，就有被「vibe coded 掉」的風險。
```
...
```
【YC 執行長開源他的 Claude 工作流，一人獨幹 60 萬行程式碼】
​把 AI 當成 23 個人的虛擬開發團隊，
​這是矽谷頂規的自動化開發 SOP，現在直接免費開源給你抄。
​
​Y Combinator 執行長 Garry Tan，
​釋出他的秘密武器「gstack」。
​他靠這套工具包，在兼顧 YC 全職工作的同時，
​60 天內寫了超過 60 萬行 production 級的程式碼。
​
​▋把空白對話框，變成軟體加工廠
​多數人用 Claude，
​只是給一段空白 Prompt 讓它自由發揮。
​Garry Tan 把 Claude Code 模組化，
​掛載了 23 個專門的虛擬角色。
​
​包含負責打槍創意的 CEO，
​鎖定軟體架構的技術主管，抓 Bug 的 QA，
​甚至是管控部署的 Release Manager。
​這套系統強迫 AI 照著「思考 → 規劃 → 開發 → 測試 → 部署」的正規流程跑。
​
​▋拒絕盲目開發，開工前先過 /office-hours
​這是這套系統最精準的設計：
​它拒絕當唯命是從的打字機。
​
​執行 `/office-hours` 指令，
​它會像個真正的技術幕僚來質疑你的需求。
​
​「你說你要寫一個日曆 App，但你其實需要的是數位秘書。」
​
​它會收斂你的發散思維，幫你把三個月的專案，
​硬是砍到只剩明天就能上線的 MVP，然後才開始出架構圖。
​
​▋多重審查防呆：不再陷入跟 AI 的除錯地獄
​自己手刻過專案就知道，AI 一但寫錯，
​修 Bug 的過程比重寫還痛苦。
​而在 gstack 的標準流程裡，
​有 `/plan-eng-review` 把關防呆，
​
​還有 `/qa` 指令讓 AI 直接開真實瀏覽器手動點擊測試，
​最後甚至得用 `/ship` 確認測試覆蓋率提升了才能合併。
​這套防護網，大幅減少了人類替 AI 擦屁股的時間。
​
​▋只要 30 秒，直接裝進你的電腦
​安裝超級簡單，這套系統基於標準化設計，
​不論你是用 Claude Code、Codex，
​Gemini CLI 還是 Cursor 都能用。

​裝完後，在你的專案底下輸入 /office-hours
​就能立刻請這群虛擬團隊出來幫你整理專案。
​
...
```bash
鍵盤快捷鍵
!	Bash 模式（直接輸入 shell 指令）
@	提及檔案 / 資料夾
\	換行（多行輸入）
Esc	中斷目前操作
Esc x2	Rewind 選單（回溯對話/程式碼）
Ctrl+R	顯示完整輸出 context
Ctrl+V	貼上圖片
Ctrl+T	查看 tasks / todos
Ctrl+B	把目前任務移到背景
Shift+Tab	Auto-accept（自動核准工具呼叫）
Slash 指令
Context 管理
/compact	壓縮 context（釋放 token）
...
/diff	查看未 commit 的變更
/plan	進入規劃模式
/pr-comments	讀取 GitHub PR 評論
/btw	附帶問題（不消耗 context）
/loop	排程重複執行任務
/voice	語音模式（20 種語言）
管理與設定
/add-dir	新增工作目錄
...
自訂指令 & Skills
自訂指令
在 .claude/commands/ 放 .md 檔，以 /my-command arg1 呼叫
# .claude/commands/my-command.md
執行任務：$ARGUMENTS
Skills
放在 .claude/skills/，由模型自動觸發，不需手動呼叫
內建 Skill 功能
/simplify	3 個平行 agent 做 code review
/batch	跨多個 worktrees 平行重構
/debug	標準除錯工作流
Headless 模式（自動化/CI）
claude --print "修復所有 lint 錯誤"
claude -p "分析程式碼" --output-format json
claude -p "重構模組" --max-turns 5
...
Agents（子代理）
在 .claude/agents/ 建立 .md 檔，獨立 context、可限制工具權限
內建模板
code-reviewer debugger data-scientist
Git Worktrees（平行開發）
...
PreToolUse	工具呼叫前
PostToolUse	工具呼叫後
UserPromptSubmit	使用者送出提示後
Notification	通知觸發
Stop	停止時
SubagentStop	子代理停止時
PreCompact	壓縮 context 前
SessionStart	Session 開始
SessionEnd	Session 結束
權限管理
Basic	常用開發指令允許，敏感檔案阻擋
Strict	最大限制，適合生產環境
Enterprise	MCP 控制，企業級管理
權限類型：allow ask deny，支援 bash command pattern matching
Checkpoint & 設定優先順序
自動追蹤變更建立回溯點，Esc x2 或 /rewind 開啟回溯選單
Bash 指令和外部編輯器的修改不會被 checkpoint 捕捉
設定載入順序（高 → 低）
/etc/claude-code/managed-settings.json（企業）
.claude/settings.local.json（專案本地）
...
- LC - tree traversal type VS LC problem pattern
  ...
  - https://github.com/yennanliu/CS_basics/commit/797b3c96c7186591ca4ae94cc5160ccf80f11574
...
- Twilio whatsapp Java
  	- https://www.1ju.org/article/spring-boot-twilio-whatsapp
    - code
      - https://github.com/eugenp/tutorials/tree/master/saas-modules/twilio-whatsapp
